                ...
                if self.check_extension(filename):
                    ...
                    if frame is not None:
                        ...
                        if ((model_type == 'OCR') and (self.modelAcvOcrSecondary == False)):
                            ...
                            Cam_File_Sink.sql_state = sql_insert                      
                            #self.send_to_upstream(json.dumps(inference_obj))
                            #f = open('/home/ucheruk/logtest.txt', 'a')
                            #f.write(json.dumps(inference_obj))
                            #f.close()
                            log_path = "/inference_local_log"
                            log_loc = os.path.join(log_path, "logtest.txt") 
                            log_data = json.dumps(inference_obj)
                            print(f"log_data:{log_data}")
                            if log_loc:
                                with open(log_loc, encoding="utf-8","a") as log_file:
                                    log_file.write(log_data)
                            else:
                                with open(log_loc,"w") as log_file: 
                                    log_file.write(log_data)
                            log_write = open(log_loc, "r")
                            print(f"log_file:{log_write.read()}")
                        ...
                        print(f'{filename} did not load. File object returned None')
